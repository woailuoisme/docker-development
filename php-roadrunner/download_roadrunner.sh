...
download_binary() {
  ...
  if [ -f "$release_file" ]; then
    echo "Archive $release_file already exists, skipping download..."
  else
  ...
  # 解压文件
  echo "Extracting RoadRunner binary..."
  ...
  # 查找解压出的二进制文件（在子目录中）
  extract_dir="$PNAME-$latest-$os-$arch"
  binary_path=""
  
  if [ -f "$extract_dir/rr" ]; then
    binary_path="$extract_dir/rr"
  elif [ -f "$extract_dir/roadrunner" ]; then
    binary_path="$extract_dir/roadrunner"
  elif [ -f "rr" ]; then
    binary_path="rr"
  elif [ -f "roadrunner" ]; then
    binary_path="roadrunner"
  fi
  
  # 检查是否找到二进制文件
  if [ -z "$binary_path" ]; then
    printf "$RED%s\n$DEFAULT" "ERROR: Failed to find binary in archive"
    exit 1
  fi
  ...
  # 移动到 /usr/local/bin/rr
  mv "$binary_path" /usr/local/bin/rr
  
  # 清理压缩包和解压目录
  rm -f "$release_file"
  rm -rf "$extract_dir"
  
  printf "$GREEN%s\n$DEFAULT" "RoadRunner binary successfully installed to /usr/local/bin/rr"
}
