...
format_caddy_configs() {
	...
	config_files="
        $CONFIG_FILE
        $(find /etc/caddy/snippets /etc/caddy/templates -name "*.conf" 2> /dev/null || true)
    "
...
validate_caddy_config() {
	...
	if caddy validate --config "$CONFIG_FILE" --adapter "$ADAPTER"; then
...
show_startup_config() {
	...
	log_info "配置文件: ${C_1}${CONFIG_FILE}${C_0}"
	log_info "适配器: ${C_1}${ADAPTER}${C_0}"
...
main() {
	...
	caddy_cmd="caddy run --config $CONFIG_FILE --adapter $ADAPTER"
	log_success "Caddy 服务启动准备完成"
	printf "${C_1}${C_G}启动命令: %s${C_0}\n\n" "$caddy_cmd"
