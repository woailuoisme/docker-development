...
setup_environment() {
    ...
    if [ -d "/home/www-data/.ssh" ] && [ -w "/home/www-data/.ssh" ]; then
        ...
        # 允许在只读文件系统或权限不足时继续
        chown -R www-data:www-data /home/www-data/.ssh 2>/dev/null || log_warning "无法修改 SSH 目录所有者 (可能是只读挂载或权限不足)"
        chmod 700 /home/www-data/.ssh 2>/dev/null || true
        [ -f "/home/www-data/.ssh/known_hosts" ] && chmod 644 /home/www-data/.ssh/known_hosts 2>/dev/null || true
    fi
